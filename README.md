...
Utility library to build and normalize SEO metadata (Next.js Metadata shape) from a single configuration object (SEOConfig). This README is written in English and uses a standard Markdown format. It includes a Table of contents, bun-only installation instructions, a Quick start for the Next.js App Router, an exhaustive feature list, and two complete layout.tsx examples (Simple and Medium) adapted to the plain RootLayout structure generated by `npx create-next-app`. The medium example includes an ALL_FEATURES object and shows how to render it so you can inspect every field the library supports.
